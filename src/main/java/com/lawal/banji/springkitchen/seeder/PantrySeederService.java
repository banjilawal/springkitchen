...
@Data
@Service
public class PantrySeederService {
    ...
    @Autowired
    public PantrySeederService(PantryItemService pantryItemService) {
    ...
    @Transactional
    public void seed(String[] names, int numberOfItems, int minReorderLevel, int maxReorderLevel) {
        if (numberOfItems <= 0 || maxReorderLevel <= 0 || minReorderLevel > maxReorderLevel)
            throw new IllegalArgumentException("Invalid arguments");
        String name = name();
        int reorderLevel = random.nextInt(maxReorderLevel - minReorderLevel) + minReorderLevel;
        int quantityInStock = random.nextInt(reorderLevel * 2);
        for (int i = 0; i < numberOfItems; i++) {
            while (pantryItemService.findByName(name) != null) name = name();
            PantryItem pantryItem = new PantryItem(null, name, quantityInStock, reorderLevel);
            System.out.println("Adding " + name + " to the pantry");
            pantryItemService.save(pantryItem);
            reorderLevel = random.nextInt(maxReorderLevel - minReorderLevel) + minReorderLevel;
            quantityInStock = random.nextInt(reorderLevel * 2);
        }
    }
}
